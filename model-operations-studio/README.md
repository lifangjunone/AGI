...
- 支持任务取消、失败重试、重启恢复和历史产物导入；任务列表使用自适应分页，桌面端保持单屏操作。
- 视频时长支持 5、10、30、60 秒；长视频按 5 秒稳定片段生成，以上一段末帧续接并自动合并。
- 任务产物使用应用内播放器，支持原生播放控制、拖动进度和直接下载，不再跳转到浏览器裸视频页。
...
默认生成 5 秒视频。10、30、60 秒任务分别拆分为 2、6、12 个 5 秒片段；每段使用 81 帧、Euler 和 tiled VAE，后续片段从上一段末帧继续，完成后由 FFmpeg 合并为一个精确目标时长的 WebM。这样可避免在 Apple Silicon 上直接生成 161-961 帧导致的内存峰值和长帧质量问题。

产物接口支持 HTTP Range 请求，已完成任务可直接在应用内弹层播放器中打开。

任务中心默认每页显示 5 条任务；当桌面可用高度达到 960px 时自动扩展到每页 8 条。页码与状态筛选联动，筛选切换会回到第一页，避免通过长页面滚动查找历史任务。
