...
function SourceManager({
  ...
  return <div className="source-overlay">
    ...
    {editing && <div className="source-editor">
      ...
      {editing.kind !== 'builtin' && <label>{editing.kind === 'http' ? 'API Base URL' : '本地项目目录'}<input value={editing.location} onChange={(event) => setEditing({ ...editing, location: event.target.value })} placeholder={editing.kind === 'http' ? 'https://example.com/api/v1' : '/Users/.../Technology_Exploration'} /></label>}
