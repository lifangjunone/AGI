...
- 新增独立 ToC 项目 `kaidan-page`（开单页）：把个人技能包装为客户定位、结果承诺、交付清单和三档报价，提供发布准备度、移动端实时售卖页、营收/平台费/产能测算、Pro 盈亏平衡提示、浏览器本地草稿、首发文案复制及独立 HTML 导出。
- 明确首版商业模型为免费发布 1 个服务并在未来真实成交后收取 3% 平台费，Pro 版目标价格为 `¥29/月`；当前版本未接入公网发布、支付和订单，不将预测收入表述为真实收入。
- 将 `kaidan-page` 升级为产品级工作流：新增微信/邮箱/电话与 CTA 配置、三档方案买家预览、价格梯度提示、交付周期展示、发布缺项定位、最近保存时间、旧草稿迁移、剪贴板失败反馈和清空确认。
- 重构手机 UX 为“编辑 / 预览”分段模式，默认直接编辑并移除遮挡表单的固定行动条；桌面继续保留实时并排预览与阶段行动条。
- 丰富 `kaidan-page` 成交能力：交付项可动态增删，新增真实案例、服务保障、FAQ、成交说服力评分，以及按目标时薪和每单工时计算的成本底价与建议主推价。
- 新增无后端买家预览链接：完整服务信息编码在 URL 中，买家可查看完整成交页、切换三档套餐、阅读 FAQ 并复制卖家联系方式；独立 HTML 导出同步包含案例、保障和 FAQ。

### verify

- `kaidan-page` 通过 9 项领域单元测试、2 项 Playwright 桌面/手机核心旅程、TypeScript/Vite 生产构建、零漏洞依赖审计和 `git diff --check`。
- 在 1120×720 与 390×844 视口验证模板切换、三档定价与预览、收入测算、HTML 下载、重置保护、移动编辑/预览切换、导航、草稿刷新恢复和无横向溢出；六维 UI 复审由 18/24 提升至 23/24。
- 丰富功能后通过 12 项领域测试和 3 项 Playwright 创建者/手机/买家旅程；买家成交页完成 390×844 全页截图验收，运行时无控制台错误。

## 2026-09-11

### fix

- 将个人主体小程序的“公众号文章助手”从无法使用的 `web-view` 改为原生
  `pages/article/index` 页面，直接调用生产文章预览接口；修复新页面样式中
  不存在的 `$color-accent-dark` 变量，`npm run build:weapp` 已验证通过。
- 修复微信虚拟支付发货通知解析：清理 XML 字段空白，并兼容官方 JSON 事件及
  `MiniGame.Payload` 嵌套订单字段；修复已部署到 Aliyun。
- 已上传小程序体验版 `1.0.11`；模拟器已验证“我的 → 公众号文章助手”进入原生页面，
 生产文章预览接口返回 HTTP 201 和动态价格 `¥0.10`，完整内容包支付仍按当前小程序首版边界暂未开放。
...
- 已通过微信开发者工具完成一笔 `¥1.00` 真实虚拟支付；微信支付确认页出现且生产服务收到
  发货通知请求。当前订单因服务器 Token 与微信后台发货推送 Token 不匹配仍为 `PENDING`，
  因此微信自动发货闭环尚未完成。
