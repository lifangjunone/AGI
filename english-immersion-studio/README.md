...
全文学习中的智能语法问答复用同一份本地 `ARK_API_KEY`，文本模型可通过
`ARK_TEXT_MODEL` 覆盖，默认使用 `doubao-seed-2-0-lite-260428`。
密钥只由 Electron 主进程读取，不会暴露给页面。打包应用也可以从系统用户数据目录
下的 `.env.local` 读取配置；模型不可用时，界面会明确标记为“本地解析”。
