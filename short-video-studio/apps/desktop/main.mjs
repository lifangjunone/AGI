...
async function firstExisting(paths) {
  for (const candidate of paths) {
    try {
      ...
      // Try the next supported local configuration path.
    }
  }
  return undefined;
}

async function configureDesktopRuntime() {
  ...
  if (configFile) process.env.FRAME60_CONFIG_FILE = configFile;
  process.env.FRAME60_DATA_DIRECTORY = path.join(userDataDirectory, "data");
}
