...
- 微信虚拟支付服务端和小程序端已接入 `short_series_goods` 道具直购，发货通知兼容官方 `WeChatPayInfo.MchOrderNo`。微信后台已开通虚拟支付并发布 `zhizhu_content_pack`（¥1.00），消息推送配置和现网 AppKey 已完成，待真机进行小额真单。
...
- 已上传正式版网络故障热修复开发版本 `1.0.7`：所有小程序接口和 WebView 地址统一使用微信后台已放行的 `https://www.lifeyoume.icu`；上线全量用户前仍需在微信后台提交审核并发布。
