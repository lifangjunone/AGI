...
ssh "$REMOTE" "rm -rf '$RELEASE' && install -d -m 0700 '$RELEASE/platform/assets' '$RELEASE/config' '$RELEASE/deploy'"
scp "$ROOT/platform/app.py" "$REMOTE:$RELEASE/platform/app.py"
scp "$ROOT/platform/auth_store.py" "$REMOTE:$RELEASE/platform/auth_store.py"
scp -r "$ROOT/platform/assets/." "$REMOTE:$RELEASE/platform/assets/"
