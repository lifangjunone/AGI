...
macOS 使用 `launchd` 每 7200 秒检查一次仓库。任务会提交工作区变更并推送到当前分支，不创建空提交；远端领先、疑似密钥或单文件超过 95 MB 时会安全停止或跳过风险文件。发现跳过项时会立即发送 macOS 系统通知，并将完整路径和原因写入 `~/Library/Logs/LifeYouMe/agi-github-sync-skipped.txt`。
